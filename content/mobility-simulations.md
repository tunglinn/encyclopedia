...
### What agent-based modeling actually is

An **agent-based model** flips the usual simulation logic. Instead of writing equations for how traffic *as a whole* behaves, you give each individual agent (a person, a vehicle, a household) its own state and its own decision rules, let thousands or millions of them interact, and let city-wide patterns fall out as a byproduct — the same "no one's in charge, but the flock turns together" idea from the Intuition section above, just applied to travel behavior instead of car-following.

Concretely, a transportation ABM agent typically carries: a home and work location, a daily activity schedule (sleep, commute, work, errands, leisure), a set of available modes (car, transit, bike, walk, ride-hail), and preferences (value of time, price sensitivity) — then the simulation lets every agent independently decide, each day, whether to keep its routine or adjust it in response to what changed (a new toll, a canceled bus route, worse-than-usual congestion).
